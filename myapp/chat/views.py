...
def getFriendsList(id):
    ...
    try:
        ...
        for id in ids:
            num = str(id)
            fr = BaseUser.objects.get(id=int(num))
            friends.append(fr)
        return friends
    except:
        return []
...
def getUserId(username):
    ...
    :return: int
    """
    use = BaseUser.objects.get(username=username)
    id = use.id
    return id
...
def search(request):
    ...
    :return:
    """
    users = list(BaseUser.objects.all())
    for user in users:
    ...
    if request.method == "POST":
        ...
        user_ls = []
        for user in users:
            if query in (user.first_name+user.last_name) or query in user.username:
                user_ls.append(user)
...
def addFriend(request, name):
    ...
    username = request.user.username
    id = getUserId(username)
    friend = BaseUser.objects.get(username=name)
    curr_user = BaseUser.objects.get(id=id)
...
def chat(request, username):
    ...
    :return:
    """
    friend = BaseUser.objects.get(username=username)
    id = getUserId(request.user.username)
    curr_user = BaseUser.objects.get(id=id)
